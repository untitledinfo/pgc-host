...
<div class="phm-store" data-columns="<?php echo (int) $args['columns']; ?>">
	...
	<div class="phm-plan-grid">
		<?php foreach ( $products as $p ) :
			$gb       = round( $p->memory / 1024, 1 );
			$disk_gb  = round( $p->disk / 1024, 1 );
			...
			<article class="phm-plan <?php echo $out ? 'is-out' : ''; ?> <?php echo ! empty( $p->featured ) ? 'is-featured' : ''; ?>" data-product="<?php echo (int) $p->id; ?>">
				...
				<header>
					...
					<div class="phm-plan-price">
						<strong><?php echo esc_html( PHM_Plans::format_price( $p->price, $p->currency ) ); ?></strong><span>/<?php esc_html_e( 'mo', 'pterodactyl-hosting' ); ?></span>
				...
				<ul>
					<li><strong><?php echo esc_html( $gb ); ?> GB</strong> <?php esc_html_e( 'RAM', 'pterodactyl-hosting' ); ?></li>
					<li><strong><?php echo (int) $p->cpu; ?>%</strong> <?php esc_html_e( 'CPU', 'pterodactyl-hosting' ); ?></li>
					<li><strong><?php echo esc_html( $disk_gb ); ?> GB</strong> <?php esc_html_e( 'NVMe', 'pterodactyl-hosting' ); ?></li>
				...
				<?php else : ?>
					<a class="phm-btn phm-btn-primary" href="<?php echo esc_url( add_query_arg( 'plan', (int) $p->id, $order_url ) ); ?>"><?php esc_html_e( 'Configure & Order', 'pterodactyl-hosting' ); ?></a>
				<?php endif; ?>
			</article>
		<?php endforeach; ?>
	</div>
